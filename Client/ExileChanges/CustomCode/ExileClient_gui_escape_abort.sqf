...
private["_display", "_controlAbort", "_controlSuicide", "_controlTitle", "_controlFiledShit", "_abortTime", "_alivePlayers", "_suicideTime"];
...
if !(alive player) then
{
	...
	else
	{
		...
		{
			_controlSuicide ctrlEnable false;
			_controlSuicide ctrlSetText format ["(Please wait (%1s)", ceil(_suicideTime - diag_tickTime)];
			_controlSuicide ctrlCommit 0;
			if (diag_tickTime > _suicideTime) exitWith
			{		
				_controlSuicide ctrlEnable true;
